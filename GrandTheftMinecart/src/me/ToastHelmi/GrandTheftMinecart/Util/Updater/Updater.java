...
public class Updater {
    ...
    public boolean shouldUpdate(String localVersion, String remoteVersion) {
    	...
    	if(localVersion.length() < remoteVersion.length()){
    		while(localVersion.length() < remoteVersion.length())
    			localVersion+="0";
    	}
    	else if(remoteVersion.length() < localVersion.length()){
    		while(remoteVersion.length() < localVersion.length())
    			remoteVersion+="0";
    	}
    	
    	long local = Long.valueOf(localVersion);
    	long remot = Long.valueOf(remoteVersion);
